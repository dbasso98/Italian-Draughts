...
public class MoveRules {
    ...
    private static void isSamePosition(Point source, Point destination) throws SamePositionException {
    ...
    private static void isCorrectDirection(Board board, Point source, Point destination) throws InvalidMoveException{
        ...
        var sourceContainsAKing = board.getTile(source.x, source.y).containsAKing();
        var movingDirection = destination.x - source.x;
        if (!sourceContainsAKing &&
                ((colorOfSourceTile == Color.WHITE && movingDirection < 0) || (colorOfSourceTile == Color.BLACK && movingDirection > 0)))
            throw new InvalidMoveException("You are moving in the opposite rowOffset!");
    }

    private static void isMovingInDiagonal(Point source, Point destination) throws NotDiagonalMoveException {
    ...
    private static void isItMovingByOneOrTwoTiles(Point source, Point destination) throws InvalidMoveException {
        var distance = Math.abs(destination.x - source.x);
        if (distance != 1 && distance != 2)
            throw new InvalidMoveException(("Checker can move only by one or two tiles!"));
    }

    static void checkTileNonEmptiness(Tile tile) throws NonEmptyTileException {
        if (tile.isNotEmpty())
            throw new NonEmptyTileException("Cannot move since tile at (" + (tile.getColumn() + 1)
                                            + "," + (tile.getRow() + 1) + ") is not empty");
    }

     static void checkTileEmptiness(Tile tile) throws EmptyTileException {
        if (tile.isEmpty())
            throw new EmptyTileException("Cannot move since tile at (" + (tile.getColumn() + 1)
                                         + "," + (tile.getRow() + 1) + ") is empty");
    }
